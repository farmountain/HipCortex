...
use hipcortex::world_model_enhanced::{
    EntityObservation, EntityState, InterventionQuery, WorldModelEnhanced,
};
...
fn bench_transition_prediction(c: &mut Criterion) {
    ...
    for num_states in [10u32, 50, 200].iter() {
        group.bench_with_input(
            BenchmarkId::new("observe_and_predict", num_states),
            num_states,
            |b, &n| {
                ...
                b.iter(|| wm.predict_next_state("state_0", "action_2"));
            },
        );
    }
    group.finish();
...
fn bench_entity_tracking(c: &mut Criterion) {
    ...
    group.bench_function("update_entity", |b| {
        ...
        b.iter(|| wm.update_entity("entity_0", obs.clone()));
    });
    group.finish();
}
...
fn bench_causal_ops(c: &mut Criterion) {
    c.bench_function("causal_intervention", |b| {
        ...
        for i in 0u32..20 {
            for j in (i + 1)..(i + 4).min(20) {
                let _ = wm.add_causal_edge(format!("node_{}", i), format!("node_{}", j));
            }
        }
    ...
    c.bench_function("uncertainty_decomposition", |b| {
        ...
        b.iter(|| wm.decompose_uncertainty("s0", "a0"));
    });
}

criterion_group!(
    benches,
    bench_transition_prediction,
