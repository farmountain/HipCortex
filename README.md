...
| **ClarifyEngine wiring** | `ReactEngine::run` returned `Err` bluntly on empty `success_factors` | Now calls `ClarifyEngine::run(EmptyAC)` → `ClarifiedBySubstrate` reloads payload and retries; the descent is the 4-rung ladder T0 environment → T1 prior art → T2 causal → T3 ask-the-user, closed by `MAX_CLARIFY_TIERS=3` and `MAX_CLARIFY_CYCLES_PER_GOAL=3` |
