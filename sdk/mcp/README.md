# HipCortex Universal MCP Server & Multi-Agent Setup Guide (`v3.12.0`)
...
| VS Code / Antigravity VSIX | **native** — `hipcortex-memory-3.12.0.vsix` (chmod +x bundled Mac/Linux bins) |
...
> **Gemini CLI:** guide-only (manual MCP). **Antigravity:** **mcp** — `hipcortex install` writes `~/.gemini/antigravity/mcp_config.json`. Also VSIX (`hipcortex-memory-3.12.0.vsix`) if VS Code–compatible. See [docs/hosts/README.md](../../docs/hosts/README.md).
...
`serverInfo.version` = **3.12.0** (product / VERSION). Full capability matrix: [docs/capabilities.md](../../docs/capabilities.md).
