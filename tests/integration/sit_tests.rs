...
async fn test_sit_end_to_end_memory_operations() {
    ...
    let action_query = server
        .query_memory("action=integration_test")
        .await
...
async fn test_sit_multiple_concurrent_operations() {
    ...
    let query_response = server
        .query_memory("action=concurrent_operation")
        .await
...
async fn test_sit_error_handling_and_recovery() {
    ...
    assert!(
        incomplete_response.status().is_client_error()
    ...
    assert!(
        health,
        "Server should still be healthy after error conditions"
    );
...
async fn test_sit_data_persistence_across_requests() {
    ...
    for (actor, action, target) in &records {
        let actor_query = server
            .query_memory(&format!("actor={}", actor))
            .await
...
async fn test_sit_performance_benchmarks() {
    ...
    assert!(
        ...
        "Single request should complete within 1 second, took {:?}",
        latency
    );
    ...
    let query_response = server
        .query_memory("actor=PerformanceTest")
        .await
    ...
    assert_eq!(query_response["total"], 1);
    assert!(
        query_latency < Duration::from_millis(500),
    ...
    assert!(
        avg_per_request < Duration::from_millis(100),
        "Average request time should be under 100ms, was {:?}",
        avg_per_request
    );

    // Verify all bulk records were added
    let bulk_query = server
        .query_memory("action=bulk_operation")
        .await
...
async fn test_sit_api_versioning_and_compatibility() {
    ...
    for (endpoint, method) in endpoints {
        ...
        if response.status().is_success() {
            if endpoint != "/health" {
                let _: serde_json::Value = response.json().await.expect(&format!(
                    "Response from {} {} should be valid JSON",
                    method, endpoint
                ));
            }
        }
