...
fn test_personalized_pagerank() {
    ...
    let _ = g.add_node("seed".into(), [0.5; 128], HashMap::new()).unwrap();
    let _ = g.add_node("a".into(), [0.1; 128], HashMap::new()).unwrap();
    let _ = g.add_node("b".into(), [0.1; 128], HashMap::new()).unwrap();
    g.add_edge("seed".into(), "a".into(), EdgeType::Causal, 0.9, 0.95).unwrap();
    g.add_edge("a".into(), "b".into(), EdgeType::Causal, 0.7, 0.8).unwrap();
    ...
    assert!(ranks.get("seed").copied().unwrap_or(0.0) > 0.10, "seed should have high rank relative to distant nodes");
    assert!(ranks.len() >= 2);
}
...
fn test_multi_hop_paths() {
    ...
    g.add_edge("x".into(), "y".into(), EdgeType::Causal, 0.5, 0.5).unwrap();
    g.add_edge("y".into(), "z".into(), EdgeType::Causal, 0.5, 0.5).unwrap();
    let paths = g.find_multi_hop_paths("x", "z", 3);
    assert!(!paths.is_empty());
    assert!(paths.iter().any(|p| p == &vec!["x".to_string(), "y".to_string(), "z".to_string()]));
}
...
fn test_detect_contradiction() {
    ...
    g.add_edge("p".into(), "q".into(), EdgeType::Causal, 0.9, 0.9).unwrap();
    ...
    assert!(is_contradict, "should detect cycle/reverse causal contradiction");
}
