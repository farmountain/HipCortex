...
fn test_full_omega_loop_with_injected_error_and_reduction() {
    ...
    let _ =
        engine
            .wm
            .observe_transition("root".into(), "act".into(), "unexpected_bad_state".into());
    ...
    assert!(
        result.is_ok(),
        "run_omega_loop must succeed for error-injected case"
    );
    ...
    assert!(
        engine.metrics.snapshots_taken >= 1,
        "snapshot taken in cycle"
    );
    ...
    assert!(
        savings >= 0.80,
        "Full loop + topo should reduce LLM calls by >=80% on injected error (auditable) got {}",
        savings
    );
    ...
    println!(
        "AUDITABLE: full omega with error reduction savings={:.1}% , metrics={:?}",
        savings * 100.0,
        engine.metrics
    );
}
