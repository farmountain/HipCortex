...
fn ppr_direct_neighbor_appears_in_results() {
    ...
    g.add_node("mem-a".into(), blank(), HashMap::new()).unwrap();
    g.add_node("mem-b".into(), blank(), HashMap::new()).unwrap();
    g.add_edge("mem-a".into(), "mem-b".into(), EdgeType::Supports, 1.0, 1.0)
        .unwrap();
...
fn ppr_direct_neighbor_ranks_above_distant_node() {
    ...
    g.add_edge("mem-a".into(), "mem-b".into(), EdgeType::Temporal, 1.0, 1.0)
        .unwrap();
    g.add_edge("mem-b".into(), "mem-c".into(), EdgeType::Temporal, 1.0, 1.0)
        .unwrap();
    g.add_edge("mem-c".into(), "mem-d".into(), EdgeType::Temporal, 1.0, 1.0)
        .unwrap();
...
fn ppr_respects_limit() {
    let mut g = CausalTopoGraph::new();
    g.add_node("mem-root".into(), blank(), HashMap::new())
        .unwrap();
    for i in 1..=15 {
        let id = format!("mem-{}", i);
        g.add_node(id.clone(), blank(), HashMap::new()).unwrap();
        g.add_edge("mem-root".into(), id, EdgeType::Supports, 1.0, 1.0)
            .unwrap();
    }
    ...
    assert_eq!(
        results.len(),
        5,
        "must respect limit=5 when more than 5 nodes are reachable"
    );
}
...
fn ppr_results_sorted_descending_by_score() {
    ...
    g.add_edge("mem-a".into(), "mem-b".into(), EdgeType::Supports, 1.0, 1.0)
        .unwrap();
    g.add_edge("mem-b".into(), "mem-c".into(), EdgeType::Supports, 1.0, 1.0)
        .unwrap();
    g.add_edge("mem-c".into(), "mem-d".into(), EdgeType::Supports, 1.0, 1.0)
        .unwrap();
...
fn ppr_with_mem_prefix_convention_strips_correctly() {
    ...
    g.add_edge(from.into(), to.into(), EdgeType::Supports, 1.0, 1.0)
        .unwrap();
    ...
    assert_eq!(
        results[0].0, to,
        "ppr returns full symbolic_id including mem- prefix"
    );
