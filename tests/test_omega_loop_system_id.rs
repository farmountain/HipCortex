...
fn test_surprise_booster_dirichlet_system_id_update() {
    ...
    assert!(
        crash_prob > 0.6,
        "Surprise booster failed to rapidly re-learn transition distribution! crash_prob: {}",
        crash_prob
    );
}
...
fn test_loop_engine_surprise_reflexion() {
    ...
    let result_low =
        engine.process_surprise_reflexion("state_init", "act_step", "state_next", 0.05);
    ...
    let result_high =
        engine.process_surprise_reflexion("state_init", "act_step", "state_anomaly", 0.45);
