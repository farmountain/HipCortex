...
impl Decision {
    ...
    pub fn approve(rationale: String, confidence: f64, predicted_resources: ResourceUsage, eu: f64) -> Self {
        Self {
            should_execute: true,
            confidence,
            rationale,
...
impl DecisionHistory {
    ...
    fn record_decision(&mut self, decision: &Decision) {
        ...
        if decision.should_execute {
            ...
            self.avg_approved_eu = ((n - 1.0) * self.avg_approved_eu + decision.expected_utility) / n;
        } else {
            self.rejected += 1;
            let n = self.rejected as f64;
            self.avg_rejected_eu = ((n - 1.0) * self.avg_rejected_eu + decision.expected_utility) / n;
        }
    }
...
impl DecisionEngine {
    ...
    pub fn evaluate(
        ...
        if context.priority >= 0.95 {
            let decision = Decision::approve(
                format!("Critical operation '{}' - executing despite risks", operation),
                context.priority,
                predicted_resources,
                eu,
            );
        ...
        let total_resource_usage = (
            predicted_resources.cpu_percent / 100.0 + // Normalize to 0-1
        ...
        let decision = Decision::approve(
            format!("Operation '{}' approved: EU={:.2}, success_rate={:.2}, health={:.2}",
    ...
    fn compute_expected_utility(
        ...
        let resource_cost = (
            predicted_resources.cpu_percent / 100.0 +
    ...
    fn adaptive_threshold(&self) -> f64 {
        ...
        (self.config.min_expected_utility + adjustment).max(0.1).min(0.5)
    }
