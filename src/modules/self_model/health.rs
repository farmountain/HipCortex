...
impl HealthAggregator {
    ...
    pub fn report_with_weight(
        ...
        self.reports.insert(module_name, HealthReport {
            health,
            weight,
    ...
    pub fn get_module_health(&self, module_name: &str) -> Result<ModuleHealth, String> {
        let report = self.reports.get(module_name)
    ...
    pub fn set_module_weight(&mut self, module_name: &str, weight: f64) -> Result<(), String> {
        let report = self.reports.get_mut(module_name)
            .ok_or_else(|| format!("No health report for module '{}'", module_name))?;
...
mod tests {
    ...
    fn test_get_module_health() {
        ...
        agg.report("test_module".to_string(), health.clone()).unwrap();
    ...
    fn test_get_overall_health_multiple_modules() {
        ...
        agg.report("module1".to_string(), create_healthy_module()).unwrap();
        agg.report("module2".to_string(), create_healthy_module()).unwrap();
        agg.report("module3".to_string(), create_healthy_module()).unwrap();
    ...
    fn test_degraded_module_detection() {
        ...
        agg.report("healthy".to_string(), create_healthy_module()).unwrap();
        agg.report("degraded".to_string(), create_degraded_module()).unwrap();
    ...
    fn test_geometric_mean_penalizes_bad_modules() {
        ...
        // Two healthy modules
        agg.report("healthy1".to_string(), create_healthy_module()).unwrap();
        agg.report("healthy2".to_string(), create_healthy_module()).unwrap();
    ...
    fn test_custom_weights() {
        ...
        agg.report_with_weight("healthy".to_string(), healthy, 10.0).unwrap();
        agg.report_with_weight("degraded".to_string(), degraded, 1.0).unwrap();

        let health_high_weight = agg.get_overall_health().unwrap();

        // Now reverse: high weight on degraded
        let mut agg2 = HealthAggregator::new();
        agg2.report_with_weight("healthy".to_string(), create_healthy_module(), 1.0).unwrap();
        agg2.report_with_weight("degraded".to_string(), create_degraded_module(), 10.0).unwrap();
    ...
    fn test_set_module_weight() {
        ...
        agg.report("test".to_string(), create_healthy_module()).unwrap();
    ...
    fn test_list_modules() {
        ...
        agg.report("mod1".to_string(), create_healthy_module()).unwrap();
        agg.report("mod2".to_string(), create_healthy_module()).unwrap();
