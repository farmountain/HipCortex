...
**Updated:** 2026-09-11 (v3.10.0 — universal server-side passive capture)
...
|---------|--------|---------|-------|
| Rust binary / Cargo | native | `cargo build` / GitHub Releases | Crate **3.10.0** (Win/macOS/Linux amd64+arm64) |
| pip (Python SDK + CLI) | native | `pip install hipcortex` | Wizard + SDK **3.10.0** (platform-agnostic) |
...
| Surface | Version |
|---------|---------|
| Cargo / pip / npm (product) | 3.10.0 |
| VS Code extension (VSIX) | 3.10.0 |
| MCP `serverInfo.version` | 3.10.0 (stamped from VERSION) |
