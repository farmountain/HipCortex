...
*(`v0.6.0` is the release these two profiles were introduced in; the artifact versions named below are the current ones.)*
...
**Python SDK** (`sdk/python/hipcortex/`, version `3.12.0` in `pyproject.toml`):
...
**MCP server** (`sdk/mcp/server.py`, version `3.12.0`): the `TOOLS` literal declares **62 tools** (62 unique names) and `RESOURCES` declares **7 resources**. Resources are auto-injected by MCP hosts at session start (no LLM tool call needed):
...
- `hipcortex://state/diff` — content-aware diff between two memory snapshots
- `hipcortex://self/health` — self-model health aggregate
- `hipcortex://experience/tiers` — Hot/Cold tier occupancy

Counts are asserted by `sdk/python/tests/test_mcp_tool_surface.py`, which parses the literals rather than trusting prose.

**VSIX** (`vscode-extension/`, version `3.12.0`): file-save passive capture exists at `extension.ts:1844` (`onDidSaveTextDocument`). Terminal passive capture wraps `vscode.window.onDidWriteTerminalData` via runtime guard (type not in bundled `@types/vscode`). Both guarded by `hipcortex.passiveCapture` config (default `true`).
